...
namespace rwl {
     ...
     * 
     * Использование:
     * В случае,  если имеется ресурс в многопоточной среде, который нужно защитить от одновременной записи,
     ...
     * 
     * \tparam Mutex Класс соответствующий ограничениям Lockable, по умолчанию std::mutex.
     * \tparam CV Класс соответствующий интерфейсу std::condition_variable.
    ...
    /** RAII для класса rwlocker. Вызывает блокировку на чтение */
